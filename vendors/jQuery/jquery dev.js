...
(fonction (globale, usine) {
	...
	if (typeof module === "object" && typeof module.exports === "object") {
		...
		// Pour les environnements CommonJS et similaires à CommonJS où une «fenêtre» appropriée
		// est présent, exécutez la fabrique et obtenez jQuery.
...
// Réussi si la fenêtre n'est pas encore définie
...
var isFunction = fonction isFunction (obj) {
      ...
      // Dans certains navigateurs, typeof renvoie "fonction" pour les éléments HTML <object>
      // (c'est-à-dire `typeof document.createElement (" object ") ===" function "`).
      // Nous ne voulons pas classer * aucun * noeud DOM en tant que fonction.
      return typeof obj === "function" && typeof obj.nodeType! == "number";
  };


var isWindow = fonction isWindow (obj) {
	...
	fonction DOMEval (code, nœud, doc) {
		...
		script.text = code;
		if (nœud) {
			pour (i dans les attributsScript préservés) {
				...
				// `jQuery.globalEval` afin qu'il puisse simuler un nœud ne contenant pas dece
				// via un objet.
...
fonction toType (obj) {
	...
	// Prise en charge: Android <= 2.3 uniquement (RegExp fonctionnel)
	return typeof obj === "object" || typeof obj === "fonction"?
		class2type [toString.call (obj)] || "objet" :
		typeof obj;
}
/ * Symbole mondial * /
// La définition de ce global dans .eslintrc.json créerait un danger d'utilisation du global
// non gardé dans un autre endroit, il semble plus sûr de définir global uniquement pour ce module
...
var
	...
	jQuery = fonction (sélecteur, contexte) {
		...
		// Besoin de l'initialisation si jQuery est appelé (juste permettre à l'erreur d'être levée s'il n'est pas inclus)
		return new jQuery.fn.init (sélecteur, contexte);
	},

	// Prise en charge: Android <= 4.0 uniquement
	// Assurez-vous que nous coupons la nomenclature et le NBSP
...
jQuery.fn = jQuery.prototype = {
	...
	// Récupère l'ensemble d'éléments mis en correspondance sous la forme d'un tableau propre
	get: fonction (num) {
		...
		// Renvoie juste le seul élément de l'ensemble
		renvoie num <0? this [num + this.length]: this [num];
	},

	// Prenez un tableau d'éléments et poussez-le sur la pile
	// (retour du nouvel ensemble d'éléments mis en correspondance)
	pushStack: fonction (elems) {

		// Créer un nouvel ensemble d'éléments correspondant à jQuery
		var ret = jQuery.merge (this.constructor (), elems);

		// Ajout de l'ancien objet sur la pile (comme référence)
		ret.prevObject = this;
	...
	map: function (rappel) {
		retourner this.pushStack (jQuery.map (this, function (elem, i) {
			return callback.call (elem, i, elem);
		}));
	},

	tranche: fonction () {
		renvoie this.pushStack (slice.apply (this, arguments));
	},
	...
	// Se comporte comme la méthode d'un tableau, pas comme une méthode jQuery.
	poussez, poussez,
	trier: arr.sort,
	épissure: arr.splice
};
...
jQuery.extend = jQuery.fn.extend = function () {
	...
	// Gère la casse lorsque la cible est une chaîne ou quelque chose (possible en copie complète)
	...
	pour (; i <longueur; i ++) {
		...
		// Traite uniquement les valeurs non nulles / non définies
		if ((options = arguments [i])! = null) {
			...
			pour (nom dans les options) {
				...
				// Prévient la pollution Object.prototype
				// Empêche une boucle sans fin
				if (name === "__proto__" || target === copy) {
					continuer;
				}

				// Recurse si nous fusionnons des objets ou des tableaux simples
				if (deep && copy && (jQuery.isPlainObject (copy) ||
					(copyIsArray = Array.isArray (copie)))) {
					src = cible [nom];

					// Assure un type correct pour la valeur source
					...
					// Ne déplacez jamais des objets originaux, clonez-les
					target [name] = jQuery.extend (deep, clone, copy);

				// N'apportez pas de valeurs indéfinies
...
jQuery.extend ({
	...
	isPlainObject: function (obj) {
		...
		// Les objets avec prototype sont simples s'ils ont été construits par une fonction d'objet globale
		Ctor = hasOwn.call (proto, "constructeur") && proto.constructor;
		renvoie le type de Ctor === "fonction" && fnToString.call (Ctor) === ObjectFunctionString;
	},
	...
	merge: fonction (premier, deuxième) {
	...
	map: fonction (elems, callback, arg) {
		...
		// Parcourez le tableau en traduisant chacun des éléments dans leurs nouvelles valeurs
		...
		// Parcourez chaque touche de l'objet,
		} autre {
			pour (i dans elems) {
				valeur = rappel (elems [i], i, arg);
		...
		// Aplatissez tous les tableaux imbriqués
	...
	// propriétés à elle donc elle doit exister.
	support: support
});
...
fonction isArrayLike (obj) {
	...
	// Prise en charge: réel iOS 8.2 uniquement (non reproductible dans le simulateur)
	// vérification `in` utilisée pour éviter les erreurs JIT (gh-2145)
	...
	type de retour === "tableau" || longueur === 0 ||
		typeof longueur === "nombre" && longueur> 0 && (longueur - 1) en obj;
}
var Sizzle =
/ *!
...
var i,
	...
	booleans = "vérifié | sélectionné | async | mise au point automatique | lecture automatique | contrôles | différer | désactivé | masqué | ismap | boucle | multiple | ouvert | lecture seule | requis | portée",
	...
	attributs = "\\ [" + espaces blancs + "* (" + identifiant + ") (?:" + espaces blancs +
		// Opérateur (capture 2)
		"* ([* ^ $ |! ~]? =)" + espace +
		// "Les valeurs d'attribut doivent être des identificateurs CSS [capture 5] ou des chaînes [capture 3 ou capture 4]"
		"* (?: '((?: \\\\. | [^ \\\\']) *) '| \" ((?: \\\\. | [^ \\\\\ "]] ) *) \ "| (" + identifiant + ")) |)" + espace +
		"* \\]",

	pseudos = ":(" + identifiant + ") (?: \\ ((" +
		// Pour réduire le nombre de sélecteurs nécessitant une tokenize dans le préFiltre, préférez les arguments:
	...
	// Espaces de début et non échappés de fin, capturant certains caractères non blancs précédant ce dernier
	rwhitespace = new RegExp (espaces blancs + "+", "g"),
	rtrim = new RegExp ("^" + espaces blancs + "+ | ((?: ^ | [^ \\\\]) (?: \\\\.) *)" + espaces blancs + "+ $", "g "),

	rcomma = new RegExp ("^" + espaces blancs + "*," + espaces blancs + "*"),
	rcombinators = new RegExp ("^" + espaces blancs + "* ([> + ~] |" + espaces blancs + ")" + espaces blancs + "*"),
	rdescend = new RegExp (espaces blancs + "|>"),
	...
	matchExpr = {
		...
		"ENFANT": nouveau RegExp ("^ :( uniquement | premier | dernier | nième | nième-dernier) - (enfant | de type) (?: \\ (" + espaces blancs +
			"* (pair | impair | (([+ -] |) (\\ d *) n |)" + espace + + "* (?: ([+ -] |)" + espace +
			"* (\\ d +) |))" + espace + "* \\) |)", "i"),
		"bool": nouveau RegExp ("^ (?:" + booléens + ") $", "i"),
		// Pour une utilisation dans les bibliothèques implémentant .is ()
		// Nous l'utilisons pour la correspondance POS dans `select`
		"needsContext": nouveau RegExp ("^" + espaces blancs + "* [> + ~] |: (pair | impair | eq | gt | lt | nth | premier | dernier) (?: \\ (" +
			espaces blancs + "* ((?: - \\ d)? \\ d *)" + espaces blancs + "* \\) |) (? = [^ -] | $)", "i")
	},

	rhtml = / HTML $ / i,
	rinputs = / ^ (?: entrée | sélection | zone de texte | bouton) $ / i,
	rheader = / ^ h \ d $ / i,

	rnative = / ^ [^ {] + \ {\ s * \ [native \ w /,

	// Sélecteurs ID ou TAG ou CLASS facilement analysables / récupérables
	rquickExpr = /^(?:#([\w- diplomatique+)|(\w+)|\.((\\--++))$/,

	rsibling = / [+ ~] /,

	// CSS s'échappe
	// http://www.w3.org/TR/CSS21/syndata.html#escaped-characters
	runescape = new RegExp ("\\\\ ([\\ da-f] {1,6}" + espaces blancs + "? | (" + espaces blancs + ") |.)", "ig"),
	funescape = function (_, escaped, escapedWhitespace) {
		var high = "0x" + échappé - 0x10000;
		// NaN signifie non-codepoint
		// Prise en charge: Firefox <24
		// Contournement interprétation numérique erronée de + "0x"
		retour haut! == haut || escapedWhitespace?
			échappé :
			élevé <0?
				// Codep BMP
				String.fromCharCode (haut + 0x10000):
				// Point de code du plan supplémentaire (paire de substitution)
				String.fromCharCode (élevé >> 10 | 0xD800, élevé & 0x3FF | 0xDC00);
	},

	// Sérialisation de chaîne / identifiant CSS
	// https://drafts.csswg.org/cssom/#common-serializing-idioms
	rcssescape = / ([\ 0- \ x1f \ x7f] | ^ -? \ d) | ^ - $ | [^ \ 0- \ x1f \ x7f- \ uFFFF \ w -] / g,
	fcssescape = fonction (ch, asCodePoint) {
		if (asCodePoint) {
		...
		// D'autres caractères ASCII potentiellement spéciaux obtiennent une barre oblique inverse
		retourner "\\" + ch;
	},

	// Utilisé pour les iframes
	// Voir setDocument ()
	// La suppression de l'encapsuleur de fonction provoque une "autorisation refusée"
	// erreur dans IE
...
essayez {
	...
	// Détecte push.apply qui échoue en silence
...
fonction Sizzle (sélecteur, contexte, résultats, graine) {
	var m, i, elem, nid, match, groups, newSelector,
		...
		// nodeType par défaut à 9, car le contexte par défaut est à documenter
	...
	si (! graine) {
		...
		if ((context? context.ownerDocument || context: preferDoc)! == document) {
		...
		if (documentIsHTML) {
			...
			if (support.qsa &&
				...
				// qSA prend en compte les éléments en dehors d'une racine de portée lors de l'évaluation d'un enfant ou
				// combinateurs descendants, ce qui n'est pas ce que nous voulons.
				// Dans de tels cas, nous contournons le comportement en préfixant chaque sélecteur du
				...
				if (nodeType === 1 && rdescend.test (sélecteur)) {
					...
					// Capture l'ID de contexte, le définissant d'abord si nécessaire
...
/ **
 * Créez des caches de valeur-clé de taille limitée
 * @returns {fonction (chaîne, objet)} Renvoie les données d'objet après les avoir stockées sur lui-même avec
 * nom de propriété la chaîne (suffixe d'espace) et (si le cache est plus grand que Expr.cacheLength)
...
function createCache () {
	...
	cache de fonction (clé, valeur) {
		// Utilisez (clé + "") pour éviter la collision avec les propriétés du prototype natif (voir problème # 157)
 ...
 * @param {Function} fn Passé l'élément créé et retourne un résultat booléen
 * /
fonction assert (fn) {
 ...
 * @param {Element} a
 * @param {Element} b
 * @returns {Number} Renvoie moins de 0 si a précède b, plus de 0 si a suit b
 * /
fonction siblingCheck (a, b) {
	...
	// Utilisez IE sourceIndex si disponible sur les deux nœuds
	if (diff) {
		return diff;
	}

	// Vérifier si b suit a
	if (cur) {
...
/ **
 * Renvoie une fonction à utiliser dans les pseudos pour: activé /: désactivé
 * @param {Boolean} désactivé true pour: désactivé; false pour: activé
 * /
fonction createDisabledPseudo (désactivée) {

	// Connu: faux positifs désactivés: fieldset [désactivé]> légende: nième type (n + 2): can-disable
	fonction de retour (elem) {
		...
		if ("forme" dans elem) {
			...
			// Vérifier l'invalidité héritée sur les éléments non désactivés pertinents:
			// * éléments listés associés au formulaire dans un champ désactivé
			...
			if (elem.parentNode && elem.disabled === false) {
				...
				// Les éléments d'option sont reportés à un optgroup parent s'il est présent
				if ("label" dans elem) {
				...
				// Utilisez la propriété de raccourci isDisabled pour rechercher les ancêtres des ensembles de champs désactivés
				return elem.isDisabled === désactivé ||

					// S'il n'y a pas d'isDisabled, vérifiez manuellement
					/ * jshint -W018 * /
		...
		// Essayez de découvrir les éléments qui ne peuvent pas être désactivés avant d'approuver la propriété désactivée.
		// Certaines victimes se font prendre dans notre filet (étiquette, légende, menu, piste), mais cela ne devrait pas
		// existe même sur eux, sans parler d'avoir une valeur booléenne.
...
/ **
 * Renvoie une fonction à utiliser dans les pseudos pour les positionnelles
 * @param {Function} fn
 * /
function createPositionalPseudo (fn) {
	return markFunction (function (argument) {
		argument = + argument;
		return markFunction (fonction (valeur de départ, correspond) {
			var j,
 ...
 * @returns {Element | Object | Boolean} Le noeud d'entrée s'il est acceptable, sinon une valeur fausse
 * /
fonction testContext (contexte) {
	retourne le contexte && typeof context.getElementsByTagName! == "non défini" && context;
}
 ...
 * /
isXML = Sizzle.isXML = fonction (elem) {
	var namespace = elem.namespaceURI,
		docElem = (elem.ownerDocument || elem) .documentElement;

	// Prise en charge: IE <= 8
	// Supposons HTML lorsque documentElement n'existe pas encore, comme le chargement intérieur d'iframes
...
setDocument = Sizzle.setDocument = fonction (nœud) {
	var hasCompare, subWindow,
		doc = nœud? node.ownerDocument || node: preferDoc;

	// Retourne tôt si le document n'est pas valide ou est déjà sélectionné
